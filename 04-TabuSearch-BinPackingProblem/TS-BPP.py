...
def generateBestNeighbor(s,ni,w,bc,lt):
    ...
    for i in range(ni):
        ...
        if (lt[i] == 0 and lt[j]==0):
            ...
            if(score > bestScore):
                ...
                bestj = j
    # Se regresan las variables de interés
    return bestSolution, bestBins, bestRepresentation, bestWeights, bestScore, visitedNeighbors, besti, bestj
...
def startTSBP(nameInstance,iterations,cota,save):
    # Función que inicia la búsqueda Tabú
    # Lectura de la instancia
    n_items,bin_capacity,weights = readInstanceModified(nameInstance,100)
    visitedNeighbors = 0
    # Cota
    L2 = save
    # Memoria Tabú y tiempo en el que un movimiento deja de ser Tabú
    tabu = [0]*n_items
    tabuTime =  math.ceil(n_items/10)
    # Solución Inicial
    ...
    while stop_condition:
        ...
        neighborSolution,neighborBins,neighborRepresentation,neighborWeights,neighborScore,neigborhs,object1,object2 = generateBestNeighbor(solution,n_items,weights,bin_capacity,tabu)
        nevaluations += neigborhs
        ...
        if (neighborScore > solution_score):
            ...
            # Actualización de los movimientos tabú
            position1 = object1
            position2 = object2
        else:
        ...
        updateList(tabu)
        tabu[position1]=tabuTime
        tabu[position2]=tabuTime
        ...
        if ((solution_bins==L2) or (count>iterations) or (not verifyList(tabu))):
            stop_condition= False
    ...
    return str(count-1)+','+best+','+str(visitedNeighbors)+','+str(nevaluations)
...
for file in instances:
    ...
    # Ciclo que permite hacer las repeticiones de la búsqueda Tabú
    for i in range(n_runs):
        ...
        r = startTSBP(nameInstance,niterations,L2,save_run)
