...
def constructGRS(initialSolution,w,bc):
    # Mejor solución encontrado en representación de agrupación
    best = [[]]
    # Lista de los mejores contenedores donde un objeto puede entrar
    possibleBins = []
    # Pesos de los posibles contenedores
    pcp = []
    # Para cada objeto de la solución
    for i in initialSolution:
        possibleBins = []
        cp = []
        pcp = []
        countBins = 0
        # Probar en cuáles contenedores de la solución actual entra
        for j in best:
            cap = 0
            for k in j:
                cap += w[k]
            if ((cap + w[i] <= bc)):
                possibleBins.append(countBins)
                pcp.append(cap + w[i])
            countBins += 1
        # Lista ordenada de los posibles contenedores
        possibleBins = [x for _,x in sorted(zip(pcp,possibleBins))]
        # Se seleccionan sólo los mejores tres que maximicen el llenado
        possibleBins=possibleBins[-3:]
        # Si no exiten posibles contenedores se crea uno
        if (len(possibleBins) == 0):
            best.append([i])
            cp.append(w[i])
        else:
            # En caso contrario, se selecciona uno aleatoriamente
            randomBin =random.randint(0,len(possibleBins)-1)
            best[possibleBins[randomBin]].append(i)
    # Proceso para cambiar la codificación de grupos en permutaciones
    l = []
    for i in best:
        for j in i :
            l.append(j)
...
def startGRASPBP(nameInstance,cota,iterations,save):
    # Función que inicia la búsqueda GRASP
    ...
    csv = 'Iteración,Número Contenedores,FBPP,Solución,Representación,Pesos,Vecinos Visitados\n -1,' + str(solution_bins) + ',' + str(solution_score) + ',' + list2string(solution) + ',' + list2string(solution_representation) + ',' + list2string(solution_weights) + ',0\n'  
    solution = constructGRS(solution,weights,bin_capacity)
    # Evaluación de la solución greedy
    solution_bins,solution_representation,solution_weights,solution_score = evaluateSolution(solution,weights,bin_capacity)
    evaluations += 1
    # csv es una variable que almacena los resultados de la mejor solución cada iteración como un texto
    csv += '0,' + str(solution_bins) + ',' + str(solution_score) + ',' + list2string(solution) + ',' + list2string(solution_representation) + ',' + list2string(solution_weights) + ',0\n'  
    #print(solution)
    ...
    while stop_condition:
        ...
        neighborSolution,neighborBins,neighborRepresentation,neighborWeights,neighborScore,neigborhs = generateBestNeighbor(solution,n_items,weights,bin_capacity)
        ...
        actual_solution = str(count) + ','  + str(solution_bins) + ',' + str(solution_score) + ',' + list2string(solution) + ',' + list2string(solution_representation) + ',' + list2string(solution_weights)
        # Se aumenta el contador de soluciones 
        count += 1
        csv += actual_solution + ',' + str(visitedN_iteration) +'\n' 
        # En caso de que el vecino no sea mejor o se cumpla alguna condición de paro
    ...
    return str(count-1)+','+best+','+str(visitedNeighbors)+','+str(evaluations)
...
for file in instances:
    ...
    results = 'Iteraciones,Numero Contenedores,FBPP,Solución,Representación,Pesos,Total Vecinos Visitados,Evaluaciones,Tiempo Ejecución \n'
    ...
    # Ciclo que permite hacer las repeticiones de la Búsqueda GRASP
    for i in range(n_runs):
        ...
        r = startGRASPBP(nameInstance,L2,niterations,save_run)
