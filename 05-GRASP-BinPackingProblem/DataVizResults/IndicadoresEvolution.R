...
setwd("C:/Users/David/Documents/GitHub/DIA-OptimizacionCombinatoria/04-GRASP-BinPackingProblem/Results")
...
for(f in files){
  ...
  pdf(file = paste("C:/Users/David/Documents/GitHub/DIA-OptimizacionCombinatoria/04-GRASP-BinPackingProblem/Results/Summary/",name2), width = 7, height = 5)
  plot(df[,c(3,9,4,1,10,5,12,11,13)] %>% melt(id.vars="Ejecución") %>% 
         ggplot(aes(x=Ejecución ,y=value, group=variable))+ geom_line() +
         ggtitle("Evolución de los indicadores de acuerdo al número de ejecución",paste(paste("GRASP (",f),")"))+
