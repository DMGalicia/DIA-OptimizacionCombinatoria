...
def generateNeighbor(s,ni,w,bc,i):
    # Función para generar todos los vecino y obtener el mejor
    # Selección aleatoria de dos objetos
...
def startSABP(nameInstance,temp,iterations,L2,save):
    # Función que inicia el Simmulated Annealing
    ...
    csv = 'Iteracion,Numero Contenedores,FBPP,Solución,representación,Pesos,Vecinos Visitados \n 0,' + str(solution_bins) + ',' + str(solution_score) + ',' + list2string(solution) + ',' + list2string(solution_representation) + ',' + list2string(solution_weights) + ',0\n'  
    ...
    while stop_condition:
        ...
        # Contador de reemplazos de soluciones
        # Variables que almacenana el vecino seleccionado
        selectedNsolution = 0
        selectedNbins = 0
        selectedNrepresentation = 0
        selectedNweights = 0
        selectedNscore = 0
        for j in range(1):
            for i in range(n_items):
                # Generar al vecino de la solución
                neighborSolution,neighborBins,neighborRepresentation,neighborWeights,neighborScore = generateNeighbor(solution,n_items,weights,bin_capacity,i)
                evaluations += 1
                visitedNeighbors += 1
                ...
                if (neighborScore > selectedNscore):
                    selectedNsolution = neighborSolution
                    selectedNbins = neighborBins
                    selectedNrepresentation = neighborRepresentation
                    selectedNweights = neighborWeights
                    selectedNscore = neighborScore
        # Se queda el último vecino seleccionado se acepta
        if (selectedNscore > solution_score):
            solution = selectedNsolution
            solution_bins = selectedNbins
            solution_representation = selectedNrepresentation
            solution_weights = selectedNweights
            solution_score = selectedNscore
        else:
            if (random.random() < t):
                solution = selectedNsolution
                solution_bins = selectedNbins
        ...
        # Se aumenta el contador de soluciones y se actualiza la temperatura
        t = temp-(count*0.1)
        count += 1
...
for file in instances:
    ...
    # Ciclo que permite hacer las repeticiones del Recocido Simulado
    for i in range(n_runs):
        ...
        r = startSABP(nameInstance,temperature,niterations,L2,save_run)
