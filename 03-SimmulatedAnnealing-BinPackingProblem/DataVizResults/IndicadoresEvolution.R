...
for(f in files){
  ...
  plot(df[,c(4,10,5,1,11,6,13,12,14)] %>% melt(id.vars="Ejecución") %>% 
    ggplot(aes(x=Ejecución ,y=value, group=variable))+ geom_line() +
    ggtitle("Evolución de los indicadores de acuerdo al número de ejecución",paste(paste("Recocido Simulado (",f),")"))+
